...
type Params = { //tipado de los parametros que se reciben por la pantalla
  id: string;
  title: string;
};
...
export default function TasksScreen() { 
  ...
  //Estados para manejar las tareas, el loading y el error
  const [tasks, setTasks] = useState<Task[]>([]);
  const [loading, setLoading] = useState(true);

  const fetchTasks = async (): Promise<Task[]> => { //funcion para simular la obtencion de tareas desde un servidor
    return new Promise((resolve) => {
      setTimeout(() => resolve(MOCK_TASKS), 1000);
    });
  };

  const handleToggle = (id: string) => { //funcion para cambiar el estado de la tarea (completada o no)
    setTasks((prev) =>
      prev.map((task) =>
        task.id === id ? { ...task, completed: !task.completed } : task,
      ),
    );
    //aqui se podria agregar una llamada a la API para actualizar el estado de la tarea en el backend
  };

  const handleMenu = (id: string) => { //funcion para mostrar el menu de la tarea (editar, eliminar, etc)
    console.log("Open menu for task:", id);
  };

  useEffect(() => { //useEffect para cargar las tareas al montar la pantalla
    const load = async () => {
      setLoading(true);
  ...
  const completedCount = tasks.filter((t) => t.completed).length; //calculo del porcentaje de tareas completadas
  const percentage =
