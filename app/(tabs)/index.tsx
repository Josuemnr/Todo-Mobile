...
export default function HomeScreen() {
  ...
  const onRefresh = async () => { //funcion para gesto de reffesh en celular
    setRefreshing(true);
    await loadLists();
    setRefreshing(false);
  };
  ...
  return (
    <SafeAreaView className="flex-1">
      <Box className="flex-1 p-4">
        ...
        {/* List */}
        {!loading && !error && ( //si carga y no hay error se muestra la lista
          <FlatList //ayuda a renderizar solo lo que estamos viendo (no toda la lista)
            data={lists}
            keyExtractor={(item) => item.id} //se pasa un ID
            renderItem={({ item }) => <TaskListCard item={item} />} //le pasamos el componente
            refreshControl={
